...
with cte as (select count(distinct r.post_id) / count(distinct a.post_id) as cnt from actions a left join removals r on a.post_id = r.post_id where a.extra = 'spam' group by a.action_date)
