...
-- cte2 as (select visited_on, sum(amount) over(order by visited_on range between interval '6' day preceding and current row) as amount, row_number() over(order by visited_on) as rn from customer)
